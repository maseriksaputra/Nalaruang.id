...
const PortalApp = () => {
    ...
    const handleToggleTemplateStatus = async (id, currentStatus) => {
        try {
            ...
            const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content');
            const response = await fetch(`/admin/builder/${id}/publish`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': csrfToken
            ...
            const data = await response.json();
            if (data.success) {
                setTemplates(templates.map(t => t.id === id ? { ...t, status: data.status, expires_at: data.expires_at } : t));
            } else {
    ...
    const handleEditSlug = async (e, id, currentSlug) => {
        ...
        try {
            const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content');
            await axios.post(`/admin/invitation-portal/${id}/slug`, { slug: newSlug.trim().toLowerCase() }, {
                headers: { 'X-CSRF-TOKEN': csrfToken }
            });
            setInvitations(invitations.map(inv => inv.id === id ? { ...inv, slug: newSlug.trim().toLowerCase() } : inv));
                                    ...
                                    {invitations.map((inv) => (
                                        ...
                                        >
                                            <div className="h-40 bg-gray-100 dark:bg-gray-800 p-0 relative overflow-hidden group-hover:opacity-90 transition-opacity">
                                                ...
                                                {inv.order_id && (
                                                    <div className="absolute top-3 left-3 px-2 py-1 backdrop-blur rounded text-[10px] font-bold text-white border uppercase z-20 shadow bg-orange-500/90 border-orange-400 flex items-center gap-1" title={`Terhubung dengan pesanan: ${inv.order?.customer_name || 'Klien VIP'}`}>
                                            ...
                                            {templateFolders[activeFolder]?.map(item => (
                                                <div key={item.id} className="bg-white dark:bg-[#1e293b] border border-gray-200 dark:border-[#334155] rounded-2xl overflow-hidden hover:border-[#475569] transition-all shadow-lg flex flex-col relative group">
                                                    ...
                                                    <div className="p-4 flex-1 flex flex-col">
                                                        ...
                                                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{item.total_uses}x digunakan</p>
                                                    </div>
                                                    
                                                    <div className="absolute inset-x-0 bottom-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity bg-gradient-to-t from-black/90 via-black/50 to-transparent flex flex-wrap gap-2 justify-end items-end z-30">
                                                        <button 
                                                            onClick={(e) => { e.stopPropagation(); handleToggleTemplateStatus(item.id, item.status); }}
