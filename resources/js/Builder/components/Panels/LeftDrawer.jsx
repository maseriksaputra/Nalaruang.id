...
const LeftDrawer = () => {
    ...
    const deleteGlobalElement = async (id, e) => {
        e.stopPropagation();
        if (await window.confirmAsync('Yakin ingin menghapus elemen ini permanen dari pustaka?', 'Hapus Elemen')) {
            try {
                const res = await apiClient.delete(`/admin/builder/global-elements/${id}`);
                if (res.data.success) {
                    fetchGlobalElements();
                }
            } catch (error) {
