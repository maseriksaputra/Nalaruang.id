                ...
                <div className="grid grid-cols-2 gap-3 mt-2">
                    ...
                    <div className="pt-4 border-t border-gray-100">
                        ...
                        <div className="grid grid-cols-3 gap-2 mb-4">
                            ...
                            <button onClick={() => alignLayer(activeLayer.id, 'center')} className={`flex flex-col items-center gap-1 p-2 border border-gray-200 rounded text-[10px] transition-colors ${Math.abs((activeLayer.style?.x || 0) - (375 - (parseInt(activeLayer.style?.width) || 100)) / 2) < 2 ? 'bg-indigo-50 text-indigo-700 border-indigo-200 shadow-sm' : 'hover:bg-gray-50 text-gray-600'}`}>
                                <svg className={`w-5 h-5 ${Math.abs((activeLayer.style?.x || 0) - (375 - (parseInt(activeLayer.style?.width) || 100)) / 2) < 2 ? 'text-indigo-600' : 'text-gray-500'}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="2" x2="12" y2="22"></line><rect x="6" y="6" width="12" height="4" rx="1"></rect><rect x="8" y="14" width="8" height="4" rx="1"></rect></svg> Tengah
                            </button>
                            <button onClick={() => alignLayer(activeLayer.id, 'right')} className={`flex flex-col items-center gap-1 p-2 border border-gray-200 rounded text-[10px] transition-colors ${Math.abs((activeLayer.style?.x || 0) - (375 - (parseInt(activeLayer.style?.width) || 100))) < 2 ? 'bg-indigo-50 text-indigo-700 border-indigo-200 shadow-sm' : 'hover:bg-gray-50 text-gray-600'}`}>
                                <svg className={`w-5 h-5 ${Math.abs((activeLayer.style?.x || 0) - (375 - (parseInt(activeLayer.style?.width) || 100))) < 2 ? 'text-indigo-600' : 'text-gray-500'}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="20" y1="2" x2="20" y2="22"></line><rect x="4" y="6" width="12" height="4" rx="1"></rect><rect x="8" y="14" width="8" height="4" rx="1"></rect></svg> Kanan
                            </button>
                        </div>
