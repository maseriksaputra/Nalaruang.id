...
<body class="font-sans text-gray-800 overflow-x-hidden selection:bg-brand-500 selection:text-white">
    ...
    <nav id="navbar" x-data="{ mobileMenuOpen: false }" class="fixed w-full z-50 transition-all duration-300 bg-white/10 backdrop-blur-sm border-b border-white/10">
        ...
        <div x-show="mobileMenuOpen" 
            ...
            <div class="px-4 pt-4 pb-6 space-y-2 flex flex-col">
                <a @click="mobileMenuOpen = false" href="{{ request()->is('/') ? '#beranda' : url('/#beranda') }}" class="block px-4 py-3 rounded-xl text-base font-bold text-gray-800 hover:text-brand-600 hover:bg-brand-50 transition">HOME</a>
                <a @click="mobileMenuOpen = false" href="{{ request()->is('/') ? '#katalog' : url('/#katalog') }}" class="block px-4 py-3 rounded-xl text-base font-bold text-gray-800 hover:text-brand-600 hover:bg-brand-50 transition">LAYANAN</a>
                <a @click="mobileMenuOpen = false" href="{{ request()->is('/') ? '#portofolio' : url('/#portofolio') }}" class="block px-4 py-3 rounded-xl text-base font-bold text-gray-800 hover:text-brand-600 hover:bg-brand-50 transition">PORTOFOLIO</a>
                <a @click="mobileMenuOpen = false" href="{{ request()->is('/') ? '#review' : url('/#review') }}" class="block px-4 py-3 rounded-xl text-base font-bold text-gray-800 hover:text-brand-600 hover:bg-brand-50 transition">CERITA KLIEN</a>
                <div class="mt-4 px-2 pt-2">
                    <a @click="mobileMenuOpen = false" href="{{ request()->is('/') ? '#kontak' : url('/#kontak') }}" class="w-full bg-brand-800 text-white px-6 py-3.5 rounded-full text-sm font-bold text-center shadow-md hover:bg-brand-900 transition block flex items-center justify-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path></svg>
                        Hubungi Kami
                    </a>
                </div>
    ...
    <div x-data class="fixed bottom-6 right-6 z-50 flex items-end gap-3">
        ...
        <button @click="$dispatch('open-chat')" class="bg-brand-600 hover:bg-brand-700 text-white w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transition transform hover:scale-110">
    ...
    <footer class="bg-brand-900 text-gray-300 py-16 border-t-[6px] border-brand-800" id="kontak">
        ...
        <div class="border-t border-brand-800/50 mt-16 pt-8 flex flex-col md:flex-row justify-between items-center max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-xs text-brand-500">
            ...
            <div class="flex gap-4 mt-4 md:mt-0">
                <a href="https://www.instagram.com/nalaruang.id?igsh=MTUyNTRjamY5MzYxOQ==" target="_blank" class="hover:text-white transition">Instagram</a>
                <a href="https://tiktok.com/@nalaruang.id" target="_blank" class="hover:text-white transition">TikTok</a>
