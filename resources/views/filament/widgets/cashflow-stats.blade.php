    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="flex items-center p-4 bg-white border border-gray-200 rounded-xl shadow-sm">
            ...
            <div>
                ...
                <p class="text-xl font-bold text-gray-900">Rp {{ number_format($stats['income'], 0, ',', '.') }}</p>
        ...
        <div class="flex items-center p-4 bg-white border border-gray-200 rounded-xl shadow-sm">
            ...
            <div>
                ...
                <p class="text-xl font-bold text-gray-900">Rp {{ number_format($stats['expense'], 0, ',', '.') }}</p>
        ...
        <!-- Card Laba Bersih -->
        <div class="flex items-center p-4 bg-white border border-gray-200 rounded-xl shadow-sm">
            <div class="flex items-center justify-center w-12 h-12 {{ $stats['net'] >= 0 ? 'bg-indigo-50 text-indigo-500' : 'bg-red-50 text-red-500' }} rounded-full mr-4">
                <!-- Icon Wallet -->
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path></svg>
            </div>
            <div>
                <p class="text-xs text-gray-500 font-medium uppercase">Laba Bersih (Nett)</p>
                <p class="text-xl font-bold text-gray-900">{{ $stats['net'] < 0 ? '-' : '' }}Rp {{ number_format(abs($stats['net']), 0, ',', '.') }}</p>
                <p class="text-[10px] {{ $stats['net'] >= 0 ? 'text-indigo-500' : 'text-red-500' }} mt-1 flex items-center">
                    @if($stats['net'] >= 0)
