...
<head>
    ...
    <title>Client Area - <?php echo e($invitation->title); ?></title>
    ...
    <script>
        window.__INVITATION__ = <?php echo json_encode($invitation, 15, 512) ?>;
...
<body class="bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-200 antialiased min-h-screen pb-24 lg:pb-0 transition-colors duration-300" x-data="{ darkMode: localStorage.getItem('darkMode') === 'true', activeTab: sessionStorage.getItem('activeTab') || 'dashboard', isMobileMenuOpen: false, qrModalOpen: false, qrUrl: '', qrName: '' }" x-init="$watch('activeTab', val => sessionStorage.setItem('activeTab', val)); $watch('darkMode', val => localStorage.setItem('darkMode', val)); if(darkMode) document.documentElement.classList.add('dark'); $watch('darkMode', val => val ? document.documentElement.classList.add('dark') : document.documentElement.classList.remove('dark'))">
    ...
    <header class="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 sticky top-0 z-50 shadow-sm">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <img src="<?php echo e(asset('logo1.png')); ?>" alt="Logo Nalaruang" class="h-10 w-auto object-contain">
            ...
            <nav class="hidden lg:flex items-center gap-1">
                ...
                <a href="/<?php echo e($invitation->slug); ?>" target="_blank" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-semibold transition shadow-md flex items-center gap-2">
        ...
        <div x-show="isMobileMenuOpen" x-collapse x-cloak class="lg:hidden bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800">
            <div class="px-4 py-3 space-y-1">
                ...
                <div class="pt-2">
                    <a href="/<?php echo e($invitation->slug); ?>" target="_blank" class="w-full px-4 py-3 bg-indigo-600 text-white rounded-lg font-semibold text-center block">Lihat Undangan</a>
                </div>
            </div>
        </div>
    </header>
    ...
    <main class="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div class="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h2 class="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white tracking-tight"><?php echo e($invitation->title); ?></h2>
        ...
        <div x-show="activeTab === 'dashboard'" x-transition.opacity.duration.300ms>
                    ...
                    <div class="flex justify-between items-start relative z-10">
                        <div>
                            ...
                            <h3 class="text-3xl font-bold text-white flex items-baseline gap-1">
                                <span class="counter" data-target="<?php echo e($totalViews); ?>">0</span>
                            </h3>
                        </div>
                ...
                <div class="stat-card p-5 rounded-2xl relative overflow-hidden group">
                    ...
                    <div class="flex justify-between items-start relative z-10">
                        <div>
                            ...
                            <h3 class="text-3xl font-bold text-white flex items-baseline gap-1">
                                <span class="counter" data-target="<?php echo e(count($links)); ?>">0</span>
                            </h3>
                        </div>
                ...
                <div class="stat-card p-5 rounded-2xl relative overflow-hidden group">
                    ...
                    <div class="flex justify-between items-start relative z-10">
                        <div>
                            ...
                            <h3 class="text-3xl font-bold text-white flex items-baseline gap-1">
                                <span class="counter" data-target="<?php echo e($rsvps->where('status', 'Hadir')->count()); ?>">0</span>
                            </h3>
                        </div>
                ...
                <div class="stat-card p-5 rounded-2xl relative overflow-hidden group">
                    ...
                    <div class="flex justify-between items-start relative z-10">
                        <div>
                            ...
                            <h3 class="text-3xl font-bold text-white flex items-baseline gap-1">
                                <span class="counter" data-target="<?php echo e($rsvps->where('status', 'Tidak Hadir')->count()); ?>">0</span>
                            </h3>
                        </div>
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    const trafficLabels = <?php echo json_encode($trafficLabels, 15, 512) ?>;
                    const trafficData = <?php echo json_encode($trafficData, 15, 512) ?>;
        ...
        <div x-show="activeTab === 'tamu'" x-cloak x-transition.opacity.duration.300ms>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden mb-8 p-6">
                ...
                <form action="/client/<?php echo e($invitation->slug); ?>/links/batch" method="POST">
                    <?php echo csrf_field(); ?>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $links; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $link): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <tr class="hover:bg-slate-50 dark:bg-slate-800/50 dark:hover:bg-slate-800/60 dark:bg-slate-800/30/50 transition group">
                                    <td class="px-6 py-4">
                                        <p class="font-bold text-slate-800 dark:text-slate-200"><?php echo e($link->guest_name); ?></p>
                                    </td>
                                    <td class="px-6 py-4 hidden md:table-cell">
                                        <div class="flex items-center gap-2">
                                            <input type="text" readonly value="<?php echo e(url('/' . $invitation->slug . '?to=' . urlencode($link->guest_name))); ?>" class="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 text-xs rounded px-2 py-1.5 w-full max-w-[250px] focus:outline-none">
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            <button @click="qrName = '<?php echo e(addslashes($link->guest_name)); ?>'; qrUrl = '<?php echo e(url('/' . $invitation->slug . '?to=' . urlencode($link->guest_name))); ?>'; qrModalOpen = true;" class="inline-flex items-center gap-1.5 px-3 py-2 bg-slate-800 hover:bg-slate-900 text-white rounded-lg text-sm font-semibold transition shadow-sm hover:shadow" title="Tampilkan QR Code">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm14 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"></path></svg>
                                                <span class="hidden sm:inline">QR</span>
                                            </button>
                                            <button onclick="shareWhatsApp('<?php echo e(addslashes($link->guest_name)); ?>')" class="inline-flex items-center gap-1.5 px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg text-sm font-semibold transition shadow-sm hover:shadow">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path></svg>
                                                <span class="hidden sm:inline">Kirim WA</span>
                                            </button>
                                            <form action="/client/<?php echo e($invitation->slug); ?>/links/<?php echo e($link->id); ?>" method="POST" class="inline-block m-0" onsubmit="confirmDelete(event, this);">
                                                <?php echo csrf_field(); ?>
        ...
        <div x-show="activeTab === 'wishes'" x-cloak x-transition.opacity.duration.300ms>
            <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col h-[600px] max-h-[80vh]">
                <div class="px-6 py-5 border-b border-slate-200 dark:border-slate-800 shrink-0">
                    <h2 class="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
                        Wishes & Konfirmasi Kehadiran
                        <span class="bg-indigo-100 text-indigo-700 dark:text-indigo-400 py-0.5 px-2.5 rounded-full text-xs"><?php echo e(count($rsvps)); ?></span>
                    </h2>
                </div>
                
                <div class="p-6 overflow-y-auto wishes-scroll flex-1 bg-slate-50 dark:bg-slate-800/50/50">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $rsvps; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rsvp): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <div class="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-100 dark:border-slate-800/50 shadow-sm mb-4 transition hover:shadow-md relative group">
                            ...
                            <div class="absolute top-4 right-4">
                                <span class="px-3 py-1 text-[10px] font-bold rounded-full uppercase tracking-wider <?php echo e($rsvp->status == 'Hadir' ? 'bg-emerald-50 text-emerald-600 border border-emerald-200' : 'bg-red-50 text-red-600 border border-red-200'); ?>">
                                    <?php echo e($rsvp->status); ?>
                            ...
                            <div class="flex items-start gap-4">
                                ...
                                <div class="w-10 h-10 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-500 shrink-0 flex items-center justify-center text-white font-bold text-sm shadow-sm">
                                    <?php echo e(strtoupper(substr($rsvp->name, 0, 1))); ?>

                                </div>
                                
                                <div class="flex-1 min-w-0 pr-16">
                                    <h4 class="font-bold text-slate-900 dark:text-white text-sm mb-0.5"><?php echo e($rsvp->name); ?></h4>
                                    <p class="text-[11px] text-slate-400 mb-2 flex items-center gap-1">
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                        <?php echo e(\Carbon\Carbon::parse($rsvp->created_at)->diffForHumans()); ?>

                                    </p>
                                    
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($rsvp->message): ?>
                                        <div class="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg border border-slate-100 dark:border-slate-800/50 text-slate-600 dark:text-slate-400 text-sm leading-relaxed relative">
                                            ...
                                            <p class="relative z-10 italic">"<?php echo e($rsvp->message); ?>"</p>
                                        </div>
                                    <?php else: ?>
...
<?php /**PATH D:\laragon\www\Undangan\resources\views/client-portal.blade.php ENDPATH**/ ?>